...
from pyecharts.charts import Bar, Grid, Page, Pie, Scatter
...
def view_data(df_data):
    ...
    fig.suptitle('出租方式与房租的箱型分布1', fontsize=16)
    ...
    # 提取整租和合租的数据
    zhengzu_data = df_data[df_data.house_rental_method == '整租'].house_rental_price.tolist()
    hezu_data = df_data[df_data.house_rental_method == '合租'].house_rental_price.tolist()

    # 创建 Boxplot 实例
    boxplot = Boxplot()

    boxplot.add_xaxis(["出租方式"])  # x 轴标签
    boxplot.add_yaxis("整租", boxplot.prepare_data([zhengzu_data]))
    boxplot.add_yaxis("合租", boxplot.prepare_data([hezu_data]))
    boxplot.set_global_opts(
        title_opts=opts.TitleOpts(title="出租方式与房租分布箱型图"),
        yaxis_opts=opts.AxisOpts(name="房租价格(元)"),  # y 轴标签

    )

    # 渲染图表
    boxplot.render("出租方式与房租.html")
    ...
    axs[0].set_title('整租方式', fontsize=13)
    axs[1].set_title('合租方式', fontsize=13)
    fig.suptitle('出租方式与房租的箱型分布2', fontsize=16)
    axs[0].set_ylabel('房租价格(元)', fontsize=12)
    axs[1].set_ylabel('')
    plt.show()"""
    ...
    plt.title('房屋出租方式统计', fontsize=14)
    plt.show()"""
    # 统计房屋出租方式
    house_rental_method = df_data.house_rental_method.value_counts()
    ...
    # 创建 Pie 实例
    pie = Pie()
    pie.add("", [list(z) for z in zip(house_rental_method.index.tolist(), house_rental_method)],
            radius=["30%", "75%"],  # 设置内外半径比例
            label_opts=opts.LabelOpts(formatter="{b}: {d}%"))  # 标签格式化，显示百分比
    pie.set_global_opts(title_opts=opts.TitleOpts(title="房屋出租方式统计"),
                        legend_opts=opts.LegendOpts(orient="vertical", pos_top="15%", pos_left="2%"))  # 设置图例位置和方向

    # 渲染图表
    pie.render("出租方式.html")
    ...
    fig.suptitle('房源特征统计', fontsize=16)
    plt.show()
    ...
    pies = (
        Pie()
            .add("出租方式", [list(z) for z in zip(house_rental_method.index.tolist(), house_rental_method)],
                 radius=["30%", "75%"],  # 设置内外半径比例
                 label_opts=opts.LabelOpts(formatter="{b}: {d}%"))  # 标签格式化，显示百分比
            .add("楼层高度", [list(z) for z in zip(house_floor.index.tolist(), house_floor)],
                 radius=["30%", "75%"],  # 设置内外半径比例
                 label_opts=opts.LabelOpts(formatter="{b}: {d}%"))  # 标签格式化，显示百分比
            .add("是否有电梯", [list(z) for z in zip(house_elevator.index.tolist(), house_elevator)],
                 radius=["30%", "75%"],  # 设置内外半径比例
                 label_opts=opts.LabelOpts(formatter="{b}: {d}%"))  # 标签格式化，显示百分比
            .add("是否提供停车位", [list(z) for z in zip(house_parking.index.tolist(), house_parking)],
                 radius=["30%", "75%"],  # 设置内外半径比例
                 label_opts=opts.LabelOpts(formatter="{b}: {d}%"))  # 标签格式化，显示百分比
            .add("是否提供燃气", [list(z) for z in zip(house_gas.index.tolist(), house_gas)],
                 radius=["30%", "75%"],  # 设置内外半径比例
                 label_opts=opts.LabelOpts(formatter="{b}: {d}%"))  # 标签格式化，显示百分比
            .add("房源性质", [list(z) for z in zip(cell_info.index.tolist(), cell_info)],
                 radius=["30%", "75%"],  # 设置内外半径比例
                 label_opts=opts.LabelOpts(formatter="{b}: {d}%"))  # 标签格式化，显示百分比
            .set_global_opts(title_opts=opts.TitleOpts(title="房源特征统计"),
                             legend_opts=opts.LegendOpts(orient="vertical", pos_top="15%", pos_left="2%")))  # 设置图例位置和方向

    # 渲染图表
    pies.render("多特征.html")
    ...
    # 添加标题及相关
    ax_station.set_title('各行政区房源出租数量统计', fontsize=16)
    ...
    bar = (
        Bar()
            .add_xaxis(house_station.index.tolist())
            .add_yaxis("出租房源数量", house_station.tolist())
            .set_global_opts(
            title_opts=opts.TitleOpts(title="各行政区房源出租数量统计", subtitle=""),
            xaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts( font_size=10)),
            yaxis_opts=opts.AxisOpts(name="出租房源数量"),
        )
    )

    # 渲染图表
    bar.render("行政区房源数量.html")
    ...
    # 添加标题及相关
    ax_tag.set_title('出租房源标签统计', fontsize=16)
    ...
    bar1 = (
        Bar()
            .add_xaxis(house_tag.index.tolist())
            .add_yaxis("", house_tag.values.tolist(), label_opts=opts.LabelOpts(position="top"))
            .set_global_opts(
            title_opts=opts.TitleOpts(title="出租房源标签统计", subtitle=""),
            xaxis_opts=opts.AxisOpts(name="房源标签", axislabel_opts=opts.LabelOpts(font_size=12, interval=0)),
            yaxis_opts=opts.AxisOpts(name="标签频次", axislabel_opts=opts.LabelOpts(font_size=12)),
        )
    )

    # 设置坐标轴刻度的字体大小
    bar1.set_series_opts(label_opts=opts.LabelOpts(font_size=8))

    # 显示数据的具体数值
    bar1.set_series_opts(
        label_opts=opts.LabelOpts(formatter="{c}", position="insideTop", font_size=8),
        markline_opts=opts.MarkLineOpts(data=[opts.MarkLineItem(type_="average")]),
    )

    # 保存为HTML
    bar1.render("标签统计.html")
    ...
    fig.suptitle('房源卧室数量统计', fontsize=16)
    # 显示数据的具体数值
    ...
    bar2 = (
        Bar()
            .add_xaxis(house_layout_zz.index.tolist())
            .add_yaxis("整租", house_layout_zz.tolist())
            .add_yaxis("合租", house_layout_hz.tolist())
            .set_global_opts(
            title_opts=opts.TitleOpts(title="房源卧室数量统计", subtitle=""),
    ...
    """
    """行政区的房价分布
    df_data_select = df_data.loc[(df_data.house_rental_method == '整租') & (df_data.house_rental_price < 10000), :]
    # 筛选特征
    ...
    # 添加标题及相关
    ax_station_price.set_title('各行政区整租房租价格分布(<10000)', fontsize=16)
    ...
    ax_station_price = sns.violinplot(x='station', y='house_rental_price', data=df_data_select)
    # 添加标题及相关
    ax_station_price.set_title('各行政区合租房租价格分布(<4000)', fontsize=16)
    ax_station_price.set_xlabel('行政区', fontsize=12)
    ax_station_price.set_ylabel('房租', fontsize=12)
    # 设置坐标轴刻度的字体大小
    ax_station_price.tick_params(axis='x', labelsize=8)
    plt.show()"""
    # 根据 house_rental_method 和房价阈值划分数据集
    df_data_hz = df_data[(df_data.house_rental_method == '合租') & (df_data.house_rental_price < 3000)]
    hz_data = df_data_hz.groupby('station')['house_rental_price'].apply(list).tolist()
    df_data_zz = df_data[(df_data.house_rental_method == '整租') & (df_data.house_rental_price < 5000)]
    zz_data = df_data_zz.groupby('station')['house_rental_price'].apply(list).tolist()
    # 获取筛选后的对应数据
    x_data_zz = df_data_zz['station'].tolist()
    y_data_zz = df_data_zz['house_rental_price'].tolist()
    x_data_hz = df_data_hz['station'].tolist()
    y_data_hz = df_data_hz['house_rental_price'].tolist()

    # 创建 Boxplot 实例
    boxplot = (
        Boxplot()
            .add_xaxis(df_data.station.unique().tolist())  # 使用整合后的 x 轴数据
            .add_yaxis("整租", zz_data)
            .add_yaxis("合租", hz_data)
            .set_global_opts(
            title_opts=opts.TitleOpts(title="各行政区合租与整租房租价格分布"),
            xaxis_opts=opts.AxisOpts(type_="category"),
            yaxis_opts=opts.AxisOpts(name="房租"),
        )
    )

    # 渲染图表
    boxplot.render("fangjia.html")
    ...
    plot_station_data(df_data, '天心', 1)
    plot_station_data(df_data, '雨花', 2)
    plot_station_data(df_data, '开福', 1)
    plot_station_data(df_data, '芙蓉', 2)
    ...
    fig.suptitle('不同特征下房租价格分布', fontsize=16)
...
def plot_station_data(df_data, area, tag):
    ...
    fig.suptitle(area + '区各区域房源价格和数量分布统计', fontsize=16)
